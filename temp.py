...
def ai_move(ai_paddle, ball, difficulty='medium'):
    ...
    if ball.speed_x > 0:
        predicted_y = ball.rect.y + (ai_paddle.rect.right - ball.rect.right) * ball.speed_y / ball.speed_x
        if predicted_y < 0:
            predicted_y = -predicted_y
        elif predicted_y > SCREEN_HEIGHT:
            predicted_y = SCREEN_HEIGHT - (predicted_y - SCREEN_HEIGHT)
    else:
        predicted_y = SCREEN_HEIGHT // 2
    
    # Move AI paddle towards predicted ball position
    if ai_paddle.rect.centery < predicted_y - 10:
        ai_paddle.speed = min(max_speed[difficulty], abs(ai_paddle.rect.centery - predicted_y) / 10)
        ai_paddle.move("down")
    elif ai_paddle.rect.centery > predicted_y + 10:
        ai_paddle.speed = min(max_speed[difficulty], abs(ai_paddle.rect.centery - predicted_y) / 10)
        ai_paddle.move("up")
